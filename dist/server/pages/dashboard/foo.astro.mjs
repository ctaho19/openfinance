...
import 'piccolore';
import { $ as $$DashboardLayout } from '../../chunks/DashboardLayout_JtV2IJw7.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "DashboardLayout", $$DashboardLayout, { "title": "Financial Order of Operations", "currentPath": "/dashboard/foo", "user": session.user, "showExploreSidebar": false }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="space-y-6"> <header> <h1 class="text-2xl font-bold text-theme-primary">Financial Order of Operations</h1> <p class="text-theme-secondary mt-1">Follow these 9 steps in order to build a strong financial foundation</p> </header> <!-- Progress Summary Card --> <div class="bg-gradient-to-r from-accent-600 to-accent-700 rounded-2xl p-6 text-white"> <div class="flex items-center justify-between"> <div class="flex items-center gap-4"> <div class="p-3 rounded-full bg-white/20"> ${renderComponent($$result2, "Target", Target, { "className": "h-8 w-8 text-white" })} </div> <div> <p class="text-sm font-medium text-white/80">Your Progress</p> <p class="text-2xl font-bold mt-1">${completedCount} of 9 Steps Complete</p> </div> </div> <div class="text-right hidden sm:block"> <p class="text-sm font-medium text-white/80">Current Focus</p> <p class="text-xl font-bold mt-1"> ${currentStep ? `Step ${currentStep.stepInfo.number}: ${currentStep.stepInfo.name}` : "All Complete!"} </p> </div> </div> <!-- Progress Bar --> <div class="mt-6 flex gap-1"> ${progress.map((step) => renderTemplate`<div${addAttribute(`h-2 flex-1 rounded-full ${step.status === "COMPLETED" ? "bg-white" : step.status === "IN_PROGRESS" ? "bg-white/50" : "bg-white/20"}`, "class")}${addAttribute(`Step ${step.stepInfo.number}: ${step.stepInfo.name}`, "title")}></div>`)} </div> </div> <!-- Steps List --> <div class="space-y-4"> ${progress.map((step, index) => {
