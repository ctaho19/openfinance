...
import 'piccolore';
import { $ as $$DashboardLayout } from '../../chunks/DashboardLayout_B-AXRllM.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "DashboardLayout", $$DashboardLayout, { "title": "Forecast", "currentPath": "/dashboard/pay-periods", "user": session.user, "showExploreSidebar": false }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="space-y-6"> <!-- Header with Period Selector --> <header class="space-y-4"> <div class="flex items-center justify-between"> <div> <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Forecast</h1> <p class="text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2"> ${renderComponent($$result2, "Calendar", Calendar, { "className": "h-4 w-4" })} ${formatPayPeriod(selectedPeriod)} </p> </div> ${isPastPeriod && renderTemplate`<span class="px-3 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full">
Past Period
...
</p> <div class="mt-2 h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden"> <div class="h-full bg-emerald-500 rounded-full transition-all duration-500"${addAttribute(`width: ${payments.length > 0 ? paidPayments.length / payments.length * 100 : 0}%`, "style")}></div> </div> </div> </div> <!-- Current Period: Link to Paycheck Plan --> ${isCurrentPeriod && payments.length > 0 && renderTemplate`<div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800 p-5"> <div class="flex items-center gap-3 mb-3"> <div class="w-10 h-10 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center"> ${renderComponent($$result2, "Wallet", Wallet, { "className": "h-5 w-5 text-blue-600 dark:text-blue-400" })} </div> <div class="flex-1"> <h3 class="font-semibold text-gray-900 dark:text-gray-100">Current Period Bills</h3> <p class="text-sm text-gray-600 dark:text-gray-400"> ${unpaidPayments.length} unpaid · ${paidPayments.length} paid
</p> </div> </div> <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
Manage your current period bill payments in the Paycheck Plan for a more detailed workflow.
</p> <a href="/dashboard/paycheck-plan" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm rounded-lg transition-colors"> ${renderComponent($$result2, "Wallet", Wallet, { "className": "h-4 w-4" })}
Open Paycheck Plan
</a> </div>`} <!-- Historical Period: Unpaid Bills (Read-Only) --> ${isPastPeriod && unpaidPayments.length > 0 && renderTemplate`${renderComponent($$result2, "SectionCard", SectionCard, { "client:visible": true, "title": "Unpaid", "subtitle": `${unpaidPayments.length} remaining`, "client:component-hydration": "visible", "client:component-path": "/Users/chris/projects/dev/openfinance/src/components/ui/section-card", "client:component-export": "SectionCard" }, { "default": async ($$result3) => renderTemplate` <div class="divide-y divide-gray-100 dark:divide-gray-800"> ${unpaidPayments.map((payment) => {
    const status = getPaymentStatus(payment.dueDate);
    return renderTemplate`<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0 group"> <div class="flex items-center gap-3"> <div class="p-1"> ${renderComponent($$result3, "Circle", Circle, { "className": "h-5 w-5 text-gray-400 dark:text-gray-500" })} </div> <div> <p class="font-medium text-gray-900 dark:text-gray-100">${payment.bill.name}</p> <p${addAttribute(`text-sm ${status === "overdue" ? "text-red-600 dark:text-red-400" : status === "due-today" ? "text-amber-600 dark:text-amber-400" : status === "due-soon" ? "text-amber-600 dark:text-amber-400" : "text-gray-500 dark:text-gray-400"}`, "class")}> ${formatDueDate(payment.dueDate)} </p> </div> </div> <span${addAttribute(`font-semibold ${status === "overdue" ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-gray-100"}`, "class")}>
$${Number(payment.amount).toLocaleString(void 0, { minimumFractionDigits: 2 })} </span> </div>`;
  })} </div> ` })}`} <!-- Future Period: Unpaid Bills (Read-Only) --> ${isFuturePeriod && unpaidPayments.length > 0 && renderTemplate`${renderComponent($$result2, "SectionCard", SectionCard, { "client:visible": true, "title": "Upcoming Bills", "subtitle": `${unpaidPayments.length} scheduled`, "client:component-hydration": "visible", "client:component-path": "/Users/chris/projects/dev/openfinance/src/components/ui/section-card", "client:component-export": "SectionCard" }, { "default": async ($$result3) => renderTemplate` <div class="divide-y divide-gray-100 dark:divide-gray-800"> ${unpaidPayments.map((payment) => {
    return renderTemplate`<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0 group"> <div class="flex items-center gap-3"> <div class="p-1"> ${renderComponent($$result3, "Circle", Circle, { "className": "h-5 w-5 text-gray-400 dark:text-gray-500" })} </div> <div> <p class="font-medium text-gray-900 dark:text-gray-100">${payment.bill.name}</p> <p class="text-sm text-gray-500 dark:text-gray-400"> ${formatDueDate(payment.dueDate)} </p> </div> </div> <span class="font-semibold text-gray-900 dark:text-gray-100">
$${Number(payment.amount).toLocaleString(void 0, { minimumFractionDigits: 2 })} </span> </div>`;
  })} </div> ` })}`} <!-- Historical/Future Period: Paid Bills (Read-Only) --> ${!isCurrentPeriod && paidPayments.length > 0 && renderTemplate`${renderComponent($$result2, "SectionCard", SectionCard, { "client:visible": true, "title": "Paid", "subtitle": `${paidPayments.length} completed`, "defaultOpen": false, "client:component-hydration": "visible", "client:component-path": "/Users/chris/projects/dev/openfinance/src/components/ui/section-card", "client:component-export": "SectionCard" }, { "default": async ($$result3) => renderTemplate` <div class="divide-y divide-gray-100 dark:divide-gray-800"> ${paidPayments.map((payment) => renderTemplate`<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0 opacity-60"> <div class="flex items-center gap-3"> <div class="p-1"> ${renderComponent($$result3, "CheckCircle2", CheckCircle2, { "className": "h-5 w-5 text-emerald-600 dark:text-emerald-400" })} </div> <div> <p class="font-medium text-gray-900 dark:text-gray-100 line-through">${payment.bill.name}</p> <p class="text-sm text-gray-500 dark:text-gray-400">${formatDueDate(payment.dueDate)}</p> </div> </div> <span class="font-semibold text-gray-900 dark:text-gray-100">
...
$${totalPaid.toLocaleString(void 0, { minimumFractionDigits: 2 })} </p> </div> <div> <p class="text-xs text-gray-500 dark:text-gray-400">Bills Paid</p> <p class="text-lg font-bold text-gray-900 dark:text-gray-100"> ${paidPayments.length} of ${payments.length} </p> </div> </div> </div>`} </div> ` })}`;
